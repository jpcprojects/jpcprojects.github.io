...
-	Restore the configuration file
    - Place the LCSVS0X server into maintenance mode
      - Log into https://IPADDRESS
      - Click on the server/node, then click [Actions/Enter maintenance mode]
      - Ensure that your config file backup is named [configBundle.tgz] and copy it to /tmp directory on the LCSVS0X server.
...
NOTE : I was receiving the error `vmware cannot synchronize host disconnected from host. reason: agent upgrade`
